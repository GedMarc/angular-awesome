...
/**
 * WaDateInputDirective
 *
 * Angular wrapper for the <wa-date-input> Web Awesome component that allows declarative usage,
 * input binding, and integration with Angular templates.
 *
 * Features:
 * - Binds all supported date-input attributes as @Input() properties
 * - Supports single and range selection modes, min/max constraints, and locale-aware segments
 * - Emits events for input, change, focus, blur, clear, show/hide
 * - Allows slot projection for label, hint, start, end, and popup icons
 * - Provides programmatic control through focus(), blur(), show(), hide(), and clear() methods
 * - Implements ControlValueAccessor for ngModel and reactive form support
 */
@Directive({
  selector: 'wa-date-input',
  standalone: true,
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => WaDateInputDirective),
      multi: true
    },
    {
...
export class WaDateInputDirective implements OnInit, OnChanges, DoCheck, ControlValueAccessor, Validator {
  // Core attributes
  ...
  // Selection / calendar configuration
  ...
  /**
   * Exposes the native date-input element for direct interaction
   */
  ...
  validate(control: AbstractControl): ValidationErrors | null {
    ...
    if (val != null && val !== '' && this.mode !== 'range') {
