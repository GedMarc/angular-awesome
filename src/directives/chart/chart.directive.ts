 ...
 * WaChartDirective
 *
 * Angular wrapper for the <wa-chart> Web Component.
 * Use this component for advanced configuration such as mixed chart types,
 * custom plugins, and direct Chart.js access.
 *
 * Features:
 * - Binds all supported chart attributes as @Input() properties
 * - Supports Chart.js configuration via the config property
 * - Enables Angular-style class and style bindings
 * - Allows slot projection for JSON configuration
...
export class WaChartDirective implements OnInit, OnChanges {
  // Properties
  @Input() type?: string;
  ...
  private applyInputs() {
    ...
    if (this.config != null) {
      (nativeEl as any).config = this.config;
    }
    if (this.plugins != null) {
      (nativeEl as any).plugins = this.plugins;
    }
  }
