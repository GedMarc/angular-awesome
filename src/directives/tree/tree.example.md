...
<script>
  ...
  selectionMode.addEventListener('change', () => {
    tree.querySelectorAll('wa-tree-item').forEach(item => item.removeAttribute('selected'));
    tree.setAttribute('selection', selectionMode.value);
  });
</script>
```
...
<style>
  .tree-with-lines { --indent-guide-width: 1px; }
</style>
```
...
<style>
  .custom-icons wa-tree-item::part(expand-button) {
    /* Disable the expand/collapse rotation animation */
    rotate: none;
  }
</style>
```
